...
Nach JEDEM Prompt: (1) diesen Skill mit neuen durablen Wahrheiten aktualisieren (Features/Entscheidungen/Dateien/Stand/Gates); (2) **relevanzgeprüft** `CLAUDE.md`/`.claude/learning` nur anpassen, wenn es die *zukünftige* Arbeitsweise effizienter/output-stärker macht. Ziel: weniger Token, mehr + hochwertigerer Output, inkrementell, ohne Fehlertoleranz. Workflows nur wo sie Tiefe bringen, nie für Einzel-Edits.

**Lern-Mechanik = CLAUDEYOOO-Loop** (`CLAUDEYOOO/scc-learn-distill|apply|consolidate.md` + `config.md`): 1-Zeiler → `.claude/learning/insights_inbox.md`; **distill** am Session-Ende → `proposals.md`; **apply** mit Policy — **auto-approve NUR EFFIZIENZ**, TECHNIK/WIRTSCHAFTLICHKEIT erst nach Owner-Review; **consolidate** monatlich/alle 5 Phasen (Token-Budget). CLAUDEYOOO enthält außerdem Handoff-/State-Templates (IMPLEMENTATION_STATE, *_HANDOFF, API_CONTRACT, SOURCE_OF_TRUTH_MATRIX, SECURITY_REVIEW, GO_LIVE_CHECKLIST, OPEN_DECISIONS) — als Vorlagen nutzen, nicht duplizieren.
