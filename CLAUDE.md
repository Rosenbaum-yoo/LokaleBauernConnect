...
10. **Selbst-Update-Automatik (Owner-Direktive 2026-06-21, verbindlich).** Nach JEDEM Prompt automatisch — ohne erneute Aufforderung: (a) Projekt-Skill `.agents/skills/lokalebauernconnect-project/SKILL.md` mit neuen, durablen Wahrheiten nachziehen (Features/Entscheidungen/Dateien/Stand/Gates); (b) **relevanzgeprüft** prüfen, ob die Erkenntnis die *zukünftige* Arbeitsweise effizienter/output-stärker macht — nur dann anpassen. **Mechanik = CLAUDEYOOO-Lernloop:** 1-Zeiler laufend → `.claude/learning/insights_inbox.md`; **distill** (Session-Ende) → `proposals.md`; **apply** per `CLAUDEYOOO/config.md` — **auto-approve NUR EFFIZIENZ**, TECHNIK/WIRTSCHAFTLICHKEIT erst nach Owner-Review; **consolidate** (monatlich/alle 5 Phasen, CLAUDE.md schlank halten, Token-Budget). Token sparen: kurze Zusammenfassungen, keine langen Erklärungen. Ziel: über die Zeit WENIGER Token, MEHR + hochwertigerer Output, inkrementell, **ohne Fehlertoleranz**.
