...
- **Stand:** 2026-06-19 · **Phase:** 1 (Fundament & Kernprodukt) · **Welle-Bezug:** WAVE_02–04, WAVE_07, WAVE_09, Phase 4 Track A
...
- **Quelle der Status-Begriffe:** `app/src/lib/types.ts` (`Availability = 'available' | 'low' | 'soon' | 'out'`), `app/src/components/AvailabilityBadge.tsx`, Datenmodell-Soll `PHASEN.md` WAVE_02 (`orgs, profiles, farms, products, availability, reservations`).
...
6. **Zeitstempel & Soft-Delete.** Jede Tabelle trägt `created_at`, `updated_at`, `deleted_at` (vgl. DB-Regeln). Statuswechsel aktualisiert `updated_at` und das jeweilige `*_at`-Feld des Zielzustands (z. B. `confirmed_at`, `picked_up_at`).
...
- **RLS:** Käufer sieht/ändert nur eigene Reservierungen (per `created_by` bzw. signiertem Token). Erzeuger sieht/ändert nur Reservierungen, deren `farm_id` zu einer Farm seiner `org_id` gehört. Staff (Support) liest org-übergreifend nur über die Support-Andockung (WAVE_07), mit Audit.
- **Abgrenzung Zahlung:** Reservierung ≠ Zahlung. Eine Reservierung kann „abgeholt" werden, ohne dass je eine SB-Zahlung (§4) entstand (Zahlung bar/vor Ort). Verknüpfung optional über `reservation_id` an der Zahlung.
...
## 3 · Hof-Verifizierung (`farms.verification_status`)
...
## 4 · SB-Zahlung (`sb_payments` / `payments`) ⭐ USP
...
- **Eligibility-Gate (P1-Guard):** Eine SB-Zahlung kann **nur** initiiert werden, wenn der Hof (a) `verification_status = verified` (§3) **und** (b) Stripe-(Connect-)Onboarding abgeschlossen + Auszahlungen aktiv hat. Fehlt eines → `403`/Sperr-Hinweis am Stand, kein PaymentIntent (kein toter Bezahl-Flow).
...
### 4.5 — Kopplung Reservierung ↔ SB-Zahlung (optional)

Eine SB-Zahlung **kann** über `reservation_id` mit einer Reservierung verknüpft sein (Käufer holt reservierte Ware am SB-Stand ab und zahlt dort), **muss** aber nicht (Spontankauf am Stand ohne Reservierung). Verknüpft gilt:
...
### Verweise
- Datenmodell-Soll: `PHASEN.md` WAVE_02 · künftige `docs/DATABASE_MODEL.md`
- Rollen/RBAC: künftige `docs/ROLE_AND_PERMISSION_MODEL.md`
- USP-Strecke: künftige `docs/spezialmodule/SB_BEZAHLUNG_USP.md` · Phase 4 Track A
