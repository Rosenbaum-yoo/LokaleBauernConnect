...
--        Erzeuger sehen ihre eigenen; Staff per expliziter Rolle.
create policy products_select on public.products
  for select using (
    deleted_at is null
    and (
      is_publicly_visible(farm_id)                 -- öffentlich gelistete Höfe (Käufer-Sicht)
      or org_id = current_org_id()                 -- eigene Org (Erzeuger)
      or has_platform_role('support')              -- Staff, explizit
    )
  );

-- SCHREIBEN/ÄNDERN: nur die eigene Org, und nur passende Rolle.
create policy products_write on public.products
  for all using (org_id = current_org_id() and has_org_role('erzeuger_admin','erzeuger_editor'))
  with check (org_id = current_org_id());
```

> `current_org_id()`, `has_org_role()`, `has_platform_role()`, `is_publicly_visible()` sind `security definer`-Hilfsfunktionen (stabil, indexierbar), die die Membership/Rollen-Auflösung **einmal** kapseln. Sie sind die einzige autoritative Quelle der wirksamen `org_id` — kein Client-Input.
...
```
   ...
   Browser / PWA  ─────▶│  Pages (statisch: web/ Landing, app/ React)   │
                        ...
                        │  │  availability · reservations · audit_log │  │
                        │  └─────────────────────────────────────────┘  │
...
- **Kern-Tabellen** (Phase 1 WAVE_02): `orgs`, `profiles`, `farms`, `products`, `availability`, `reservations`, `audit_log` (+ kommerzielle: `subscriptions`/`entitlements`, SB-Bezahlung: `sb_payments`/`receipts`). Jede mit `org_id`, Zeitstempeln, `deleted_at`, RLS. Schema-Wahrheit: `docs/DATABASE_MODEL.md`.
...
- **Daten:** Volltabellen-Scans sind bei dieser Größe unkritisch — trotzdem **alle Indizes ab Migration #1** (kein Nachrüst-Schuld). Pflicht-Indizes: `org_id`, `farms(plz)`/Geo, `products(farm_id, availability)`, `reservations(farm_id, created_at)`, `audit_log(org_id, created_at)`.
