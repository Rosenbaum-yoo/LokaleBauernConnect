...
| `app/index.html` | `lang="de"`, `meta viewport`, `theme-color #faf7ee`, `description`, **Apple-Touch-Icon + SVG-Icon**, **OG-/Twitter-Meta** real vorhanden. Manifest-Link + SW-Registration werden von `vite-plugin-pwa` beim Build **automatisch injiziert** (nicht statisch im HTML). | PWA-Grundausstattung erfüllt; weitere SEO-/Social-Politur additiv möglich. |
...
| `app/public/` | `_headers`, `_redirects`, `icon.svg`. Manifest + Service Worker werden **vom Plugin generiert** (nicht statisch in `public/`). | PWA real umgesetzt über `vite-plugin-pwa` (s. u.); Icon ist `icon.svg` (`purpose any` + `maskable`) — separate PNG-Icons (192/512) bewusst nicht angelegt. |
| `app/package.json` | `build = tsc --noEmit && vite build`; deps: react 18, leaflet, qrcode.react, supabase-js, **`vite-plugin-pwa`**; Test-Runner **Vitest real vorhanden** (`test`/`test:watch`). | A11y-/Audit-Tooling (jest-axe, Lighthouse-CI) bewusst **nicht** ergänzt — A11y über Verhaltens-/Komponententests in der Vitest-Suite. |
| `app/vite.config.ts` | `outDir: 'dist'`, `sourcemap:false`, Port 5409; **`VitePWA({ registerType: 'autoUpdate', manifest, workbox })`** real konfiguriert. | PWA über `vite-plugin-pwa` (Workbox-Precache `**/*.{js,css,html,svg,woff2,webmanifest}`, `navigateFallback: /index.html`). |

> **Umsetzungsentscheidung (repo-genau, Stand 2026-06-20):** Die PWA ist über **`vite-plugin-pwa`** (VitePWA + Workbox) realisiert — `registerType: 'autoUpdate'`, plugin-generiertes Manifest + Service Worker, **Workbox-Precache nur für statische Assets** (`**/*.{js,css,html,svg,woff2,webmanifest}`), `navigateFallback: /index.html`. **Sicherheits-Anker bleibt:** Es werden **keine `*.supabase.co`-Daten** in den Precache aufgenommen (kein API-Glob) — RLS-/mandantengebundene Wahrheit kommt nie aus dem SW. (Hinweis: Ein zuvor erwogener handgeschriebener SW wurde zugunsten des wartungsärmeren Plugins verworfen; die schlankere, plugin-basierte Realität ist hier als Wahrheitspunkt festgehalten — kein stiller Architekturwechsel.)
