...
| `app/package.json` | Scripts `dev/build (tsc --noEmit && vite build)/preview/typecheck`; **noch kein Test-Runner** | Phase O führt den **offiziellen Test-Runner** ein (Deno-Tests für SQL/Edge + Vitest für Frontend); bis dahin gilt `npm run build`/`typecheck` + `supabase db reset` als Verifikation. |
